...
apiRouter.post('/reports', async (req, res, next) => {
    ...
    try {
        const newReport = await createReport({ title, location, description, password });
        res.send( newReport );
    } catch(error) {
        next(error)
    } 
...
apiRouter.delete('/reports/:reportId', async (req, res, next) => {
    try {
        ...
        const closedReport = await closeReport(req.params.reportId, password);
...
apiRouter.post('/reports/:reportId/comments', async (req, res, next) => {
    ...
    try {
        const newComment = await createReportComment(req.params.reportId, { content });
